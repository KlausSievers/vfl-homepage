...
public class FileSystemDocumentDao implements IDocumentDao {
  ...
  public FileSystemDocumentDao(String directory) {
  ...
  /**
   * Inserts a document to the archive by creating a folder with the UUID of the
   * document. In the folder the document is saved and a properties file with
   * the meta data of the document.
   *
   * @see
   * org.murygin.archive.dao.IDocumentDao#insert(org.murygin.archive.service.Document)
   */
  @Override
   ...
   *
   * @see
   * org.murygin.archive.dao.IDocumentDao#findByPersonNameDate(java.lang.String,
   * java.util.Date)
   */
  @Override
  ...
  private Document loadFromFileSystem(String uuid) throws IOException {
    ...
    document.setFileData(Files.readAllBytes(Paths.get(getFilePath(metadata))));

    File thumbnail = Paths.get(getDirectoryPath(document), "preview.png").toFile();
    if (thumbnail.exists()) {
      document.setThumbnail(ImageIO.read(thumbnail));
    }
    return document;
  }
  ...
  private void savePreview(Document document) throws IOException {
    String path = getDirectoryPath(document);
    RenderedImage previewImg = document.getThumbnail();

    if (previewImg != null) {
      ImageIO.write(previewImg, "png", new FileOutputStream(new File(new File(path), "preview.png")));
    }
  }
